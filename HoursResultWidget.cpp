...
HoursResultWidget::HoursResultWidget(const char *conninfo, StudentSession *session, WContainerWidget *parent) :
...
void HoursResultWidget::showRange(Wt::WDate& startDate, Wt::WDate& endDate)
{		
	...
	Hourss hourss = dbsession.find<Hours>().where("stud_id = ?").	bind(studid).where("hours_date BETWEEN ? AND ?").bind(startDate).bind(endDate);
	...
	Hoursc hoursc = dbsession.find<Hours>().where("stud_id = ?").bind(studid).where("hours_date BETWEEN ? AND ?").bind(startDate).bind(endDate);
	...
	result->addWidget(new Wt::WText("<h3>Print Community Service Hours</h3>"));
	result->addWidget(table);
	result->addWidget(finaltotal);
	result->addWidget(new Wt::WBreak());

	Wt::WText *fullName = new Wt::WText("Supervisor Full Name:");
	Wt::WText *fullNameLine = new Wt::WText("_____________________________________________");

	Wt::WText *signature = new Wt::WText("Supervisor Signature:");
	Wt::WText *signatureLine = new Wt::WText("_____________________________________________");
	

	sig->addWidget(fullName);
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(fullNameLine);
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(signature);
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(new Wt::WBreak());
	sig->addWidget(signatureLine);
